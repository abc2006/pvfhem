...
sub fsp_devel_Initialize($)
{
    ...
    $hash->{AttrList} =
        $readingFnAttributes . " mode:manual,automatic interval:2,5,10";
}

#####################################
sub fsp_devel_Define($$)
{
    my ( $hash, $def ) = @_;
    my @a = split( "[ \t][ \t]*", $def );
...
sub fsp_devel_Notify($$)
{
    ...
    my $name = $hash->{NAME};
    Log3( $name, 4,
        "fsp_devel ($name) - fsp_devel_Notify  Line: " . __LINE__ );
    return if ( IsDisabled($name) );
    my $devname = $dev->{NAME};
    my $devtype = $dev->{TYPE};
    my $events  = deviceEvents( $dev, 1 );
    Log3( $name, 4,
        "fsp_devel ($name) - fsp_devel_Notify - not disabled  Line: "
            . __LINE__ );
    return if ( !$events );

    if (grep /^ATTR.$name.mode/,
        @{$events} or grep /^INITIALIZED$/,
        @{$events}
        )
    {
        Log3( $name, 4,
            "fsp_devel ($name) - fsp_devel_Notify change mode to AttrVal($name,mode) _Line: "
                . __LINE__ );
        $hash->{MODE} = AttrVal( $name, "mode", "automatic" );
    }
    if (grep /^ATTR.$name.interval/,
        @{$events} or grep /^INITIALIZED$/,
        @{$events}
        )
    {
    ...
    Log3( $name, 4,
        "fsp_devel ($name) - fsp_devel_Notify got events @{$events} Line: "
            . __LINE__ );
    fsp_devel_prepareRequests("prepare:$name")
        if (
        grep /^INITIALIZED$/,
...
sub fsp_devel_Set($@)
{
    ...
    my $name = $hash->{NAME};
    my $usage =
        "Unknown argument $a[1], choose one of cmd timer:noArg S005LON:0,1 S012FPADJ S005ED:feed_power_enable,feed_power_disable S013FPRADJ S013FPSADJ S013FPTADJ";
    my $ret;
    my $minInterval = 30;
    ...
    if ( $a[1] eq "?" )
    {
        ...
        $hash->{VERSION} = 1.13;
        Log3( $name, 5,
            "fsp_devel argument set fragezeichen _Line:" . __LINE__ );
        return $usage;
    }
    if ( $a[1] eq "cmd" )
    {
    ...
    if ( defined( $a[2] ) && $a[2] ne "" )
    {
        ...
        Log3( $name, 5,
            "fsp_devel setvalue $setvalue_hex _Line: " . __LINE__ );
    }

    if ( $setcmd eq "S005LON" )
    {    #en/dis-able power supply to load
        my $push = "5e533030354c4f4e" . $setvalue_hex . "0d";
        push( @{ $hash->{helper}{addCMD} }, $push );
        Log3( $name, 5, "fsp_devel_set push: $push _Line: " . __LINE__ );
    }
    elsif ( $setcmd eq "S012FPADJ" )
    {    # Feeding Grid power Calibrationi
            #S012FPADJm,nnnn
            #m = direktion => +=1; -=0
            ...
            && abs( $a[2] ) < 1000 )
        {
            ...
            Log3( $name, 5,
                "fsp_devel setFn S012FPADJ a2 $a[2] _Line: " . __LINE__ );
            my @digits = split( "", $a[2] );
            my $value;
            foreach (@digits)
            {
            ...
            Log3( $name, 5,
                "fsp_devel setFn S012FPADJ laenge $lenval _Line: "
                    . __LINE__ );
            $sendstring .= $value;
            $sendstring .= "0d";
            Log3( $name, 5,
                "fsp_devel setFn S012FPADJ $sendstring _Line: " . __LINE__ );
        }
        ## den Befehl wegsenden
        push( @{ $hash->{helper}{addCMD} }, $sendstring );
        Log3( $name, 5,
            "fsp_devel_set push: $sendstring _Line: " . __LINE__ );
    ...
    {    #
            ...
            && abs( $a[2] ) < 1000 )
        {
            ...
            Log3( $name, 5,
                "fsp_devel setFn S013FPRADJ a2 $a[2] _Line: " . __LINE__ );
            my @digits = split( "", $a[2] );
            my $value;
            foreach (@digits)
            {
            ...
            my $lenval = length($value);
            Log3( $name, 5,
                "fsp_devel setFn S013FPADJ laenge $lenval _Line: "
                    . __LINE__ );
            $sendstring .= $value;
            $sendstring .= "0d";
            Log3( $name, 5,
                "fsp_devel setFn S013FPADJ $sendstring _Line: " . __LINE__ );
        }
        ## den Befehl wegsenden
        push( @{ $hash->{helper}{addCMD} }, $sendstring );
        Log3( $name, 5,
    ...
    {    #
            ...
            && abs( $a[2] ) < 1000 )
        {
            ...
            Log3( $name, 5,
                "fsp_devel setFn S013FPSADJ a2 $a[2] _Line: " . __LINE__ );
            my @digits = split( "", $a[2] );
            my $value;
            foreach (@digits)
            {
    ...
    {    #
            ...
            && abs( $a[2] ) < 1000 )
        {
            ...
            Log3( $name, 5,
                "fsp_devel setFn S013FPTADJ a2 $a[2] _Line: " . __LINE__ );
            my @digits = split( "", $a[2] );
            my $value;
            foreach (@digits)
            {
                ...
                {
                    Log3( $name, 5,
                        "fsp_devel setFn S013FPTADJ digits $_ _Line: "
                            . __LINE__ );
                    $value .= unpack "H*", $_;
                }
            }
            ...
            my $lenval = length($value);
            Log3( $name, 5,
                "fsp_devel setFn S013FPTADJ laenge $lenval _Line: "
                    . __LINE__ );
            $sendstring .= $value;
            $sendstring .= "0d";
            Log3( $name, 5,
                "fsp_devel setFn S013FPTADJ $sendstring _Line: " . __LINE__ );
        }
        ## den Befehl wegsenden
        push( @{ $hash->{helper}{addCMD} }, $sendstring );
        Log3( $name, 5,
...
sub fsp_devel_Get($@)
{
    ...
    if ( $getcmd eq "P004GOV" )
    {    # Query AC input Voltage acceptable range for feed Power
        push( @{ $hash->{helper}{addCMD} }, "5e50303034474f560d" );
    }
    if ( $getcmd eq "P004GOF" )
    {    # Query AC input frequency acceptable range for feed Power
        push( @{ $hash->{helper}{addCMD} }, "5e50303034474f460d" );
    }
    if ( $getcmd eq "P005OPMP" )
    {    #Query the maximum output power - durchgestrichen..
        push( @{ $hash->{helper}{addCMD} }, "5e503030354f504d500d" );
    }
    if ( $getcmd eq "P005GPMP" )
    {    # Query the maximum output Power for feeding Grid
        push( @{ $hash->{helper}{addCMD} }, "5e5030303547504d500d" );
    }
    if ( $getcmd eq "P006MPPTV" )
    {    #Query Solar input MPP acceptable Range
        push( @{ $hash->{helper}{addCMD} }, "5e3030364d505054560d" );
    }
    if ( $getcmd eq "P004LST" )
    {    #Query LCD Sleeping Time
        push( @{ $hash->{helper}{addCMD} }, "5e503030344c53540d" );
    }

    if ( $getcmd eq "P003SV" )
    {    # Query Solar input Voltage acceptable Range
        push( @{ $hash->{helper}{addCMD} }, "5e5030303353560d" );
    }
    if ( $getcmd eq "P003DI" )
    {    # Query default Value for changeable Parameter
        push( @{ $hash->{helper}{addCMD} }, "5e5030303344490d" );
    }
    if ( $getcmd eq "P005BATS" )
    {    # Query Battery Setting
    ...
    if ( $getcmd eq "P004MAR" )
    {    # Query Maschine Adjustable range
        push( @{ $hash->{helper}{addCMD} }, "5e503030344d41520d" );
    }
    if ( $getcmd eq "P004CFS" )
    {    # Query current Fault Status
        push( @{ $hash->{helper}{addCMD} }, "5e503030344346530d" );
    }
    if ( $getcmd eq "P006HFSnn" )
    {    # Query history fault parameter
        push( @{ $hash->{helper}{addCMD} }, "" );

        # unwichtig ?!?
    }
    if ( $getcmd eq "P005HECS" )
    {    # Query Energy control Status
        push( @{ $hash->{helper}{addCMD} }, "5e50303035484543530d" );
    }
    if ( $getcmd eq "P006GLTHV" )
    {    # Query AC input long-lime highest average Power
        push( @{ $hash->{helper}{addCMD} }, "5e50303036474c5448560d" );
    }
    if ( $getcmd eq "P004FET" )
    {    #Query first generated energy save time
        push( @{ $hash->{helper}{addCMD} }, "5e503030344645540d" );
    }
    if ( $getcmd eq "P003FT" )
    {    # Query wait time for Feed Power
        push( @{ $hash->{helper}{addCMD} }, "5e5030303346540d" );
    }
    if ( $getcmd eq "P005ACCT" )
    {    #Query AC charge time bucket
        push( @{ $hash->{helper}{addCMD} }, "5e50303035414343540d" );
    }
    if ( $getcmd eq "P005ACLT" )
    {    #Query AC supply load time bucket
        push( @{ $hash->{helper}{addCMD} }, "5e5030303541434c540d" );
    }
    if ( $getcmd eq "P006FPADJ" )
    {    #Query feeding grid power calibration
        push( @{ $hash->{helper}{addCMD} }, "5e50303036465041444a0d" );
    ...
    if ( $getcmd eq "P006FPPF" )
    {    #Query feed in Power factor
        push( @{ $hash->{helper}{addCMD} }, "5e50303036465050460d" );
    }
    if ( $getcmd eq "P007EMINFO" )
    {    #Query feed in Power factor
        push( @{ $hash->{helper}{addCMD} }, "5e50303037454d494e464f0d" );
        Log3( $name, 3,
            "fsp_devel($name) adding $getcmd to cmdqueue _Line:" . __LINE__ );
        Log3( $name, 4,
            "fsp_devel($name) cmdqueue scalar(@{$hash->{helper}{addCMD}}  _Line:"
                . __LINE__ );
    }
    if ( $getcmd eq "P005AAPF" )
    {    #query auto-adjust PF with power Information (rot gefärbt)
    ...
    {
        Log3( $name, 3,
            "fsp_devel($name) getcmd in automatic mode" . __LINE__ );
...
sub fsp_devel_prepareRequests
{
    ...
    if ( $calltype eq 'prepare' )
    {
        ...
        foreach my $key ( keys %requests_02 )
        {
            ...
            Log3( $name, 4,
                "fsp_devel($name) prepareRequests_02 key $key value $requests_02{$key}_Line:"
                    . __LINE__ );
        }
        my $now = gettimeofday();
        $hash->{helper}{timer1} = $hash->{helper}{timer1}
            // 0;    ## if helper_timer is not defined, use 0
        if ( $now - $hash->{helper}{timer1} > 30 )
        {
            foreach my $key ( keys %requests_30 )
            {
                ...
                Log3( $name, 4,
                    "fsp_devel($name) prepareRequests_30 key $key value $requests_30{$key}_Line:"
                        . __LINE__ );
            }
...
sub fsp_devel_sendRequests
{
    ...
    my $hash = $defs{$name};
    Log3( $name, 4,
        "fsp_devel sendRequests calltype $calltype _Line:" . __LINE__ );

    # anzahl der Items bestimmen
    my $len = @{ $hash->{actionQueue} };
    Log3( $name, 4,
        "fsp_devel addCMD length $len before adding _Line:" . __LINE__ );
    foreach ( @{ $hash->{helper}{addCMD} } )
    {
        Log3( $name, 4, "fsp_devel addCMD adding item $_ _Line:" . __LINE__ );
        push(
            @{ $hash->{actionQueue} },
            shift( @{ $hash->{helper}{addCMD} } )
        );
    }
    $len = @{ $hash->{actionQueue} };
    Log3( $name, 4,
        "fsp_devel addCMD length $len after adding _Line:" . __LINE__ );

    my $length = @{ $hash->{actionQueue} };
    Log3( $name, 4,
        "fsp_devel actionQueue length $length _Line:" . __LINE__ );
    if ( $length > 0 )
    {
        #nehme den ersten Wert aus dem array und sende ihn
        my $req = shift( @{ $hash->{actionQueue} } );
        Log3( $name, 4,
            "fsp_devel sendRequests sende $req _Line:" . __LINE__ );
        ...
        InternalTimer( gettimeofday() + 1,
            'fsp_devel_sendRequests', "watchdog:$name" );
...
sub fsp_devel_Read($$)
{
    ...
    Log3( $name, 5,
        "fsp_devel($name) helper_recv: $hash->{helper}{recv}  _Line: "
            . __LINE__ );
...
##	Log3($name,5, "fsp_devel checksum: $1 _Line: " . __LINE__ );
        my $dev = pack( 'H*', $1 );
        Log3( $name, 5,
            "fsp_devel($name) rcvMessage: $1 _Line: " . __LINE__ );
...
## fsp_devel_check_sum($response);
    ...
    Log3( $name, 5,
        "fsp_devel check_sum length(received_ascii): $debu3 _Line:"
            . __LINE__ );
    if ( length( $hash->{helper}{received_ascii} ) == 0 )
    {
        Log3( $name, 5,
            "fsp_devel fehler:received ist empty or undefined: $hash->{helper}{received_ascii} _Line:"
                . __LINE__ );
        return (0);
    }
    ...
    Log3( $name, 5,
        "fsp_devel check_sum response: $hash->{helper}{received_ascii} _Line:"
            . __LINE__ );
    ...
    Log3( $name, 5,
        "fsp_devel check_sum payload: $payload _Line:" . __LINE__ );
    ...
    Log3( $name, 5,
        "fsp_devel check_sum length(response)-5-len (sollte 0 sein): $debu _Line:"
            . __LINE__ );
    ...
    {
        Log3( $name, 0,
            "fsp_devel check_sum CRC mismatch before $crc1 _Line:"
                . __LINE__ );
        $crc1 = $crc1 + 0x01;
        $crc1 = $crc1 << 8;
        Log3( $name, 0,
            "fsp_devel check_sum CRC mismatch after $crc1 _Line:"
                . __LINE__ );
        $digest   = $crc1 + $crc2;
        $writelog = 1;
    }
    ...
    {
        Log3( $name, 0,
            "fsp_devel check_sum CRC mismatch before $crc2 _Line:"
                . __LINE__ );
        $crc2 = $crc2 + 0x01;
        Log3( $name, 0,
            "fsp_devel check_sum CRC mismatch after $crc2 _Line:"
                . __LINE__ );
        $digest   = $crc1 + $crc2;
        $writelog = 1;
    }
    ...
    {
        Log3( $name, 0,
            "fsp_devel check_sum check: $digest == $debu2 _Line:"
                . __LINE__ );
    }
    return (0) unless $digest == unpack( 'n', $crc );
    Log3( $name, 5,
        "fsp_devel check_sum check: $digest == $debu2 _Line:" . __LINE__ );
    my $order = $label . $len;
...
sub fsp_devel_analyzeAnswer
{
    ...
    my $name = $hash->{NAME};
    Log3( $name, 3,
        "fsp_devel analyzeAnswer jetzt wird ausgewertet _Line:" . __LINE__ );
    my ( $order, $data ) = fsp_devel_check_sum($hash);
    Log3( $name, 5,
        "fsp_devel analyzeAnswer order: $order _Line:" . __LINE__ );
    ...
    if ( $order eq "D025" )
    {    ##  Query Series Number, P003ID
        readingsSingleUpdate( $hash, "SerialNumber", $splits[0], 1 );
    }
    elsif ( $order eq "D101" )
    {    ## Query Power Status, P003PS, 5e5030303350530d

        readingsBeginUpdate($hash);
        readingsBulkUpdate( $hash, "Solar_input_power_1", int( $splits[0] ),
            1 );
        readingsBulkUpdate( $hash, "Solar_input_power_2", int( $splits[1] ),
            1 );
        readingsBulkUpdate( $hash, "Solar_input_power_total",
            int( $splits[0] + $splits[1] ), 1 );
##	 	readingsBulkUpdate($hash,"Battery_Power",$splits[2],1); Field is empty in original answer
        readingsBulkUpdate( $hash, "AC_input_active_Power_R",
            int( $splits[3] ), 1 );
        readingsBulkUpdate( $hash, "AC_input_active_Power_S",
            int( $splits[4] ), 1 );
        readingsBulkUpdate( $hash, "AC_input_active_Power_T",
            int( $splits[5] ), 1 );
        readingsBulkUpdate( $hash, "AC_input_active_Power_total",
            int( $splits[6] ), 1 );
        readingsBulkUpdate( $hash, "AC_output_active_Power_R",
            int( $splits[7] ), 1 );
        readingsBulkUpdate( $hash, "AC_output_active_Power_S",
            int( $splits[8] ), 1 );
        readingsBulkUpdate( $hash, "AC_output_active_Power_T",
            int( $splits[9] ), 1 );
        readingsBulkUpdate(
            $hash,
        ...
        readingsBulkUpdate( $hash, "AC_output_appearent_Power_R",
            int( $splits[11] ), 1 );
        readingsBulkUpdate( $hash, "AC_output_appearent_Power_S",
            int( $splits[12] ), 1 );
        readingsBulkUpdate( $hash, "AC_output_appearent_Power_T",
            int( $splits[13] ), 1 );
        readingsBulkUpdate(
            $hash,
            "AC_output_appearent_Power_total",
            int( $splits[14] ), 1
        );
        readingsBulkUpdate( $hash, "AC_output_power_percentage",
            int( $splits[15] ), 1 );
        $splits[16] = $splits[16] == 0 ? "disconnected" : "connected";
        readingsBulkUpdate( $hash, "AC_output_connect_status", $splits[16],
            1 );
        $splits[17] = $splits[17] == 0 ? "idle" : "working";
        readingsBulkUpdate( $hash, "Solar_input_work_status_1", $splits[17],
            1 );
        $splits[18] = $splits[18] == 0 ? "idle" : "working";
        readingsBulkUpdate( $hash, "Solar_input_work_status_2", $splits[18],
            1 );
        $splits[19] =
              $splits[19] == 0 ? "standby"
            : $splits[19] == 1 ? "charge"
            :                    "discharge";
        readingsBulkUpdate( $hash, "Battery_Power_direction", $splits[19],
            1 );
        $splits[20] =
              $splits[20] == 0 ? "standby"
            : $splits[20] == 1 ? "AC->DC"
    ...
    {    ## Query Warning Status, P003WS, 5e5030303357530d
        readingsBeginUpdate($hash);
        readingsBulkUpdate( $hash, "Solar_input_loss_1", int( $splits[0] ),
            1 );
        readingsBulkUpdate( $hash, "Solar_input_loss_2", int( $splits[1] ),
            1 );
        readingsBulkUpdate( $hash, "Solar_input_too_high_1", $splits[2], 1 );
        readingsBulkUpdate( $hash, "Solar_input_too_high_1",
            int( $splits[3] ), 1 );
        readingsBulkUpdate( $hash, "Battery_under_voltage",
            int( $splits[4] ), 1 );
        readingsBulkUpdate( $hash, "Battery_low_voltage", int( $splits[5] ),
            1 );
        readingsBulkUpdate( $hash, "Battery_disconnected", int( $splits[6] ),
            1 );
        readingsBulkUpdate( $hash, "Battery_over_voltage", int( $splits[7] ),
            1 );
        readingsBulkUpdate( $hash, "Battery_low_in_hybrid_mode",
            int( $splits[8] ), 1 );
        readingsBulkUpdate( $hash, "Grid_voltage_too_high",
            int( $splits[9] ), 1 );
        readingsBulkUpdate( $hash, "Grid_voltage_too_low",
            int( $splits[10] ), 1 );
        readingsBulkUpdate( $hash, "Grid_frequency_too_high",
            int( $splits[11] ), 1 );
        readingsBulkUpdate( $hash, "Grid_frequency_too_low",
            int( $splits[12] ), 1 );
        readingsBulkUpdate(
            $hash,
            "Grid_long_time_average_too_high",
            int( $splits[13] ), 1
        );
        readingsBulkUpdate(
            $hash,
        ...
        readingsBulkUpdate(
            $hash,
            "AC_input_frequency_out_of_range",
            int( $splits[15] ), 1
        );
        readingsBulkUpdate( $hash, "AC_input_island", int( $splits[16] ), 1 );
        readingsBulkUpdate( $hash, "AC_input_phase_dislocation", $splits[17],
            1 );
        readingsBulkUpdate( $hash, "Overtemperature", $splits[18], 1 );
        readingsBulkUpdate( $hash, "Overload",        $splits[19], 1 );
        readingsBulkUpdate( $hash, "Emergency_power_off_active", $splits[20],
            1 );
    ...
    {    ## Query feed in grid calibration, P006FPADJ, 5e50303036465041444a0d
        if ( scalar(@splits) == 8 )
        {
            readingsBeginUpdate($hash);
            my $factor = undef;
            ...
            readingsBulkUpdate(
                $hash,
                "grid_power_calibration_total",
                int( $splits[1] ) * $factor, 1
            );
            $factor = $splits[2] == 0 ? -1 : 1;
            readingsBulkUpdate( $hash, "grid_power_calibration_R",
                int( $splits[3] ) * $factor, 1 );
            $factor = $splits[4] == 0 ? -1 : 1;
            readingsBulkUpdate( $hash, "grid_power_calibration_S",
                int( $splits[5] ) * $factor, 1 );
            $factor = $splits[6] == 0 ? -1 : 1;
            readingsBulkUpdate( $hash, "grid_power_calibration_T",
                int( $splits[7] ) * $factor, 1 );
        ...
        elsif ( scalar(@splits) == 6 )
        {
            ...
            readingsBulkUpdate( $hash, "EMINFO_default_feedin_power",
                int( $splits[1] ), 1 );
            readingsBulkUpdate( $hash, "EMINFO_actual_PV_power",
                int( $splits[2] ), 1 );
            readingsBulkUpdate( $hash, "EMINFO_actual_feedin_power",
                int( $splits[3] ), 1 );
            readingsBulkUpdate(
                $hash,
                "EMINFO_actual_reserved_power",
                int( $splits[4] ), 1
            );
    ...
    {    ## Query Working Mode, P004MOD, 5e503030344d4f440d
        my $state;
        if ( $splits[0] == 0 )
        {
            $state = "Power on Mode";
    ...
    { ## Query the maximum output Power for feeding Grid P005GPMP, 5e5030303547504d500d
        readingsSingleUpdate( $hash, "max_feeding_power_pw",
            int( $splits[0] ), 1 );

        Log3( $name, 4,
            "fsp_devel analyzeAnswer splits0: $splits[0] _Line:" . __LINE__ );

    }
    elsif ( $order eq "D110" )
    {    ## Query Power Status, P003GS, 5e5030303347530d
        my $leng = @splits;
        Log3( $name, 4,
            "fsp_devel analyzeAnswer length $leng _Line:" . __LINE__ );
        Log3( $name, 4,
            "fsp_devel analyzeAnswer 0 $splits[0] _Line:" . __LINE__ );

        readingsBeginUpdate($hash);
        readingsBulkUpdate( $hash, "Solar_input_voltage_1",
            int( $splits[0] ) / 10, 1 );
        readingsBulkUpdate( $hash, "Solar_input_voltage_2",
            int( $splits[1] ) / 10, 1 );
        readingsBulkUpdate( $hash, "Solar_input_current_1",
            int( $splits[2] ) / 100, 1 );
        readingsBulkUpdate( $hash, "Solar_input_current_2",
            int( $splits[3] ) / 100, 1 );
        readingsBulkUpdate( $hash, "Battery_voltage", int( $splits[4] ) / 10,
            1 );
        readingsBulkUpdate( $hash, "Battery_capacity", int( $splits[5] ), 1 );
        my $powerdirection =
            ReadingsVal( $name, "Battery_Power_direction", "" );
        my $battpower;
        my $powerdir = 1;
        ...
        readingsBulkUpdate( $hash, "Battery_current",
            ( $splits[6] / 10 ) * $powerdir, 1 );
        readingsBulkUpdate( $hash, "Battery_Power", $battpower, 1 );
        readingsBulkUpdate( $hash, "AC_input_voltage_R", int( $splits[7] ),
            1 );
        readingsBulkUpdate( $hash, "AC_input_voltage_S", int( $splits[8] ),
            1 );
        readingsBulkUpdate( $hash, "AC_input_voltage_T", int( $splits[9] ),
            1 );
        readingsBulkUpdate( $hash, "AC_input_frequency", $splits[10] / 100,
            1 );
        readingsBulkUpdate( $hash, "AC_input_current_R", int( $splits[11] ),
            1 );
        readingsBulkUpdate( $hash, "AC_input_current_S", int( $splits[12] ),
            1 );
        readingsBulkUpdate( $hash, "AC_input_current_T", int( $splits[13] ),
            1 );
        readingsBulkUpdate( $hash, "AC_output_voltage_R",
            int( $splits[14] ) / 10, 1 );
        readingsBulkUpdate( $hash, "AC_output_voltage_S",
            int( $splits[15] ) / 10, 1 );
        readingsBulkUpdate( $hash, "AC_output_voltage_T",
            int( $splits[16] ) / 10, 1 );
        readingsBulkUpdate( $hash, "AC_output_frequency", $splits[17] / 100,
            1 );
       ...
       #readingsBulkUpdate($hash,"AC_output_current_T",int($splits[20])/10,1);
        ...
        readingsBulkUpdate( $hash, "inner_temperature", int( $splits[21] ),
            1 );
        readingsBulkUpdate( $hash, "component_max_temperature",
            int( $splits[22] ), 1 );
        readingsBulkUpdate( $hash, "external_batt_temperature", $splits[23],
            1 );
    ...
    else
    {
        ...
        readingsSingleUpdate( $hash, "1_communication",
            $hash->{helper}{received_hex}, 1 );
    }
